...
const activeCount = computed(() => bookingStore.activeBookings(userId.value, 'customer').length)
...
<template>
  <div class="px-5 py-4 space-y-6">
    ...
    <div
      ...
      class="bg-primary-600 text-white rounded-2xl p-4 flex items-center justify-between cursor-pointer"
    >
      <div>
        <p class="font-semibold text-sm">{{ activeCount }} Active Booking{{ activeCount > 1 ? 's' : '' }}</p>
    ...
    <div class="bg-gradient-to-r from-primary-500 to-primary-700 rounded-3xl p-5 text-white relative overflow-hidden">
      ...
      <button
        ...
        class="bg-white text-primary-700 text-xs font-bold px-4 py-2 rounded-xl"
      >
        Browse Services →
      </button>
    </div>
    ...
    <div>
      ...
      <div class="grid grid-cols-3 gap-3">
        <button
          ...
          :class="['card p-3 flex flex-col items-center gap-1.5 text-center active:scale-95 transition-all', meta.bgColor]"
        >
    ...
    <div>
      <div class="flex items-center justify-between mb-3">
        <h3 class="font-display font-bold text-slate-900 dark:text-white">Top Professionals</h3>
        <button @click="router.push('/customer/services')" class="text-xs text-primary-600 font-medium">
          See all
        </button>
      </div>
      <div v-if="serviceStore.isLoading" class="space-y-3">
        <AppSkeleton height="h-24" rounded="rounded-3xl" :count="2" />
      </div>
